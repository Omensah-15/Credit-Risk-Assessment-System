...
# New model files from your training
...
# -------------------- Model Loading --------------------
@st.cache_resource
def load_models() -> Tuple[Optional[object], Optional[object], Optional[List[str]]]:
    """Load model, scaler, and feature columns from the new model files."""
    try:
        # Try to load the complete credit_model.pkl first (if it exists)
        if os.path.exists(CREDIT_MODEL_FILE):
            artifacts = joblib.load(CREDIT_MODEL_FILE)
...
# -------------------- Preprocessing --------------------
EMPLOYMENT_MAPPING = {'employed': 0, 'self-employed': 1, 'unemployed': 2, 'student': 3, 'retired': 4}
...
def preprocess_inference_data(input_data, feature_columns: List[str], scaler=None) -> pd.DataFrame:
    """
    Preprocess input data for model inference using the new model's features.
    """
    ...
    df['collateral_present'] = df.get('collateral_present', pd.Series()).map(COLLATERAL_MAPPING).fillna(0).astype(int)

    # Create all engineered features that match the training
    ...
    df['employment_status_employed'] = (df['employment_status'] == 0).astype(int)
    df['employment_status_self_employed'] = (df['employment_status'] == 1).astype(int)
    df['employment_status_unemployed'] = (df['employment_status'] == 2).astype(int)
    df['employment_status_student'] = (df['employment_status'] == 3).astype(int)
    df['employment_status_retired'] = (df['employment_status'] == 4).astype(int)
    
    df['education_level_high_school'] = (df['education_level'] == 0).astype(int)
    df['education_level_diploma'] = (df['education_level'] == 1).astype(int)
    df['education_level_bachelor'] = (df['education_level'] == 2).astype(int)
    df['education_level_master'] = (df['education_level'] == 3).astype(int)
    df['education_level_phd'] = (df['education_level'] == 4).astype(int)
    
    df['loan_purpose_business'] = (df['loan_purpose'] == 0).astype(int)
    df['loan_purpose_crypto'] = (df['loan_purpose'] == 1).astype(int)
    df['loan_purpose_car'] = (df['loan_purpose'] == 2).astype(int)
    df['loan_purpose_education'] = (df['loan_purpose'] == 3).astype(int)
    df['loan_purpose_home'] = (df['loan_purpose'] == 4).astype(int)
    
    df['collateral_yes'] = (df['collateral_present'] == 1).astype(int)
    df['collateral_no'] = (df['collateral_present'] == 0).astype(int)
...
def predict_single(input_dict: dict) -> Tuple[float, int, str]:
    """
    Predict for a single applicant using the new model.
    ...
    if not feature_columns:
        raise RuntimeError("Feature columns not found. Please check the models directory.")
    
    processed = preprocess_inference_data(input_dict, feature_columns, scaler)
...
model, _, feature_columns = load_models()
if model is not None:
    st.sidebar.success(f"✓ Model loaded ({len(feature_columns)} features)")
else:
